...
Create a typed and indexed sparse array to hold JuMP variables. The array can be created 
empty or have variables for a provided index set. The array name will
be registered with the model.

## Example
 ```julia
@indexedvariable(m, x[cars,year])  
idx = [("volvo",1989), ("nissan",1988), ("nissan", 1991)]
@indexedvariable(m, x[cars,year] for (cars,year) in idx)
 ```    
"""
macro indexedvariable(args...)
    args = _reorder_parameters(args)
    m = args[1]
